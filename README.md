...
1.  **[FFmpeg](https://www.gyan.dev/ffmpeg/builds/):** Необходим для конвертации аудио и рендера `.mp4`. Рекомендуется стабильная версия (full build). Включает `ffmpeg.exe` и `ffprobe.exe`.
2.  **[MKVToolNix](https://mkvtoolnix.download/downloads.html):** Утилиты `mkvmerge.exe` и `mkvextract.exe` используются для работы с MKV-контейнерами. Доступна portable-версия.
3.  **[qBittorrent](https://www.qbittorrent.org/download.php):** Используется для скачивания торрентов через Web API.
4.  **NUGEN Audio AMB** *(необязательно)*: Используется для нормализации громкости аудиодорожки. Коммерческое ПО, приобретается отдельно.
...
Инструменты можно установить двумя способами:

-   **Portable (рекомендуется):** Скачайте FFmpeg и MKVToolNix, распакуйте и положите `ffmpeg.exe`, `ffprobe.exe`, `mkvmerge.exe`, `mkvextract.exe` в папку `tools/` рядом с `DubbingTool.exe`. Программа найдёт их автоматически.
-   **Системная установка:** Установите FFmpeg и MKVToolNix обычным способом и добавьте их в системный `PATH`, либо укажите пути вручную в настройках программы.
...
3.  При первом запуске откроется **Мастер настройки**, который проведёт вас через три шага:
    -   **Проверка инструментов:** Программа автоматически найдёт FFmpeg и MKVToolNix в папке `tools/`, рядом с exe или в PATH. Если что-то не найдено — укажите путь вручную или скачайте по ссылкам в окне мастера.
    -   **qBittorrent:** Укажите данные для подключения к Web API qBittorrent (хост, порт, логин, пароль). Можно пропустить.
    -   **Пресет рендера:** Выберите пресет для рендера `.mp4` (рекомендуется `NVIDIA` или `Intel` при наличии соответствующей видеокарты).
4.  Мастер настройки можно вызвать повторно из меню: `Мастер настройки`.
