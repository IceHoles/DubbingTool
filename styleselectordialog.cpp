...
void StyleSelectorDialog::analyzeFile(const QString &filePath)
{
    ...
    while(!in.atEnd()) {
        ...
        if (line.startsWith("[Fonts]") || line.startsWith("[Graphics]")) break; // Дальше нет смысла искать
        ...
        if (line.startsWith("Style:")) {
            ...
            if (parts.size() > 4) {
                ...
                if(!style.isEmpty()) foundStyles.insert(style);
                if(!actor.isEmpty()) foundActors.insert(actor);
            }
        }
    }
