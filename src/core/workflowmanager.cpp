    ...
    double bframeOverlap = 0.0;
    {
        ...
        if (m_processManager->executeAndWait(ffprobePath, probeArgs, probeOutput))
        {
            ...
            if (ok && seg1Duration > m_concatKfBeforeTbStart)
            {
                ...
                emit logMessage(QString("Concat рендер: сег.1 фактическая длительность %1с, B-frame хвост %2с — "
...
void WorkflowManager::runAssProcessing()
{
    ...
    if (m_isSrtMasterDecoupled && hasOverrideSubs && m_template.createSrtMaster)
    {
        ...
        {
            emit logMessage("Сценарий: диалоги не используются, надписи не указаны. Генерируем только ТБ.",
                            LogCategory::APP);
        ...
        {
            emit logMessage("Сценарий: диалоги не используются, надписи не указаны. Генерация ТБ отключена. Выходной "
                            "файл будет без субтитров и надписей",
                            LogCategory::APP);
        }
