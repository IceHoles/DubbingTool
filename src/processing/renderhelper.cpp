...
RenderHelper::RenderHelper(RenderPreset preset, const QString& outputMp4Path, ProcessManager* procManager,
                           QObject* parent)
    : QObject(parent), m_preset(preset), m_outputMp4Path(outputMp4Path), m_procManager(procManager)
{
}
...
void RenderHelper::startCheck()
{
    ...
    QStringList ffprobeArgs = {
        "-v",   "quiet",        "-select_streams", "v:0", "-show_entries", "stream=bit_rate", "-print_format",
        "json", m_outputMp4Path};

    if (!m_procManager->executeAndWait(ffprobePath, ffprobeArgs, jsonData) || jsonData.isEmpty())
    {
        emit logMessage(
            "Не удалось получить информацию о MP4 файле для проверки битрейта (процесс ffprobe не вернул данные).");
        emit finished(RerenderDecision::Accept, m_preset);
        this->deleteLater();
        return;
    }
    ...
    emit logMessage(QString("Целевой битрейт: %1 kbps. Фактический: %2 kbps. Отклонение: %3%.")
                        .arg(targetBitrate)
                        .arg(qRound(actualBitrateKbps))
                        .arg(qRound(deviation * 100)));
